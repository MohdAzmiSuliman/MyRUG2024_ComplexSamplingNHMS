...
```{r}
sampel_weight <- simnhmsds_final %>% 
  left_join(., 
            district_adw %>% select(district, district_adw)) %>% 
  as_survey_design(id = 1, 
                   weights = district_adw) %>% 
  group_by(gender, agegp, ethnicity) %>% 
  summarise(sampel_adw = survey_total(success), 
            .groups = "drop")  %>% 
...
```{r}
ps_finalweight <- sampel_weight %>% 
  select(-sampel_adw_se) %>% 
  left_join(., 
            ps_pop, 
            by = join_by(gender, agegp, ethnicity)) %>% 
  mutate(fpop = population / sampel_adw, 
         final_weight = 1/fpop)
